...
class GenFieldPSF(object):
    ...
    def __init__(self, sim_psf_toolkit, max_order=45, xlim=1e3, ylim=1e3, n_bins=35,
                 lim_max_wfe_rms=None, verbose=0):
        # Input attributes
    ...
    def normalize_field(self):
        ...
        max_wfe_rms = np.max([max_wfe_corner_1, max_wfe_corner_2, max_wfe_corner_3, max_wfe_corner_4])

        if max_wfe_rms > self.lim_max_wfe_rms:
            if self.verbose:
                print('WFE_RMS %.4f [um] is exceeding from max value: %.4f [um]. Normalizing.'%(
    ...
    def zernike_coeff_map(self, x, y):
        ...
        if dif_wfe_rms < 0:
            print("WARNING: Position (%d,%d) posses an WFE_RMS of %f.\n"%(x, y, self.sim_psf_toolkit.max_wfe_rms - dif_wfe_rms))
            print("It exceeds the maximum allowed error (max WFE_RMS=%.4f [um])"%(self.sim_psf_toolkit.max_wfe_rms))
    ...
    def inspect_field_wfe_rms(self, mesh_bins=20, save_img=False):
        ...
        wfe_rms_field = np.array([self.compute_wfe_rms(_x,_y)
