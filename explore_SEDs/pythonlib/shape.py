...
'''blah, blah, blah.
Important note to self: the basic ellipticity here were thoroughly checked against Pierre-Antoine's and
are 100% safe to use.

The whole deweighted/matched thing not so much so - basically you ran into some problems when
testing it that you never quite managed to fix, iirc if you used either it worked great but 
both at the same time messed things up or something. Basically consider both of these unsafe and
deprecated; if you ever want to use them again, check our your fork of sf_tools on GitHub which I
think is outdated compared to this but Elon knows. Also you had unpushed commits on Sundancer but
you were not sure about the second one (the first was a factor 2 correction that is done below I 
believe). Good luck Morgan'''
...
class Ellipticity():
    ...
    #
    def __init__(self, data, sigma=1000, centroid=None, weights=None, 
                 moments=None, match=False, n_iter=10):
        """Class initialiser
        ...
        weights : np.ndarray, optional
            Weighting function (default is 'None')
        ...
        if not isinstance(moments, type(None)):
            ...
            self.get_ellipse()
        elif not isinstance(weights, type(None)):
