...
class CCDMissalignmentCalculator(object):
    ...
    def initialise_polygons(self):
        ...
        for it in range(self.tiles_data.shape[2]):
            ...
            for jj in range(self.scaled_data.shape[0]):
                self.scaled_data[jj,0:2,it] = self.scale_position_to_wavediff_reference(self.scaled_data[jj,0:2,it])
            # Build polygons point list
            curr_polygon = [[_x, _y] for _x, _y in zip(self.scaled_data[:,0,it], self.scaled_data[:,1,it])]
            # Build and add polygons to list
            self.ccd_polygons.append(mpltPath.Path(curr_polygon))

    def initialise_kdtree(self):

        flattened_points = np.zeros((int(self.scaled_data.shape[0] * self.scaled_data.shape[2]), 2))
    ...
    def precompute_CCD_planes(self):
        ...
        for it in range(self.scaled_data.shape[2]):
            ...
            normal = np.array([uy*vz-uz*vy, uz*vx-ux*vz, ux*vy-uy*vx])  # u_cross_v
    ...
    def compute_z_from_plane_data(pos, normal, d):
        ...
        z = (-normal[0] * pos[0] - normal[1] * pos[1] - d) * 1. / normal[2]
    ...
    def scale_position_to_tile_reference(self, pos):
        ...
        scaled_x = scaled_x * (self.tiles_x_lims[1] - self.tiles_x_lims[0]) + self.tiles_x_lims[0]

        scaled_y = (pos_y - self.y_lims[0]) / (self.y_lims[1] - self.y_lims[0])
        scaled_y = scaled_y * (self.tiles_y_lims[1] - self.tiles_y_lims[0]) + self.tiles_y_lims[0]
    ...
    def scale_position_to_wavediff_reference(self, pos):
        ...
        scaled_x = (pos_x - self.tiles_x_lims[0]) / (self.tiles_x_lims[1] - self.tiles_x_lims[0])
        scaled_x = scaled_x * (self.x_lims[1] - self.x_lims[0]) + self.x_lims[0]

        scaled_y = (pos_y - self.tiles_y_lims[0]) / (self.tiles_y_lims[1] - self.tiles_y_lims[0])
    ...
    def check_position_wavediff_limits(self, pos):
        ...
        if (pos[0] < self.x_lims[0] or pos[0] > self.x_lims[1]) or (
            ...
            raise ValueError("Input position is not within the WaveDiff focal plane limits.")
    ...
    def check_position_tile_limits(self, pos):
        ...
        if (pos[0] < self.tiles_x_lims[0] or pos[0] > self.tiles_x_lims[1]) or (
            ...
            raise ValueError("Input position is not within the tile focal plane limits.")
    ...
    def get_ccd_from_position(self, pos):
        ...
        ccds_results = np.array([ccd_polygon.contains_points(pos)[0] for ccd_polygon in self.ccd_polygons])
