...
class MeshHelper:
    ...
    @staticmethod
    def build_mesh(x_lims, y_lims, grid_points, num=None, endpoint=True):
        """Build Mesh.
...
class CoordinateHelper:
    ...
    @staticmethod
    def clip_and_scale_limits(x, y, x_lims, y_lims):
        """Clip and Scale coordinate limits.
        
        A method to clip values to the specified limits and 
        scale them to fit within the range [-1, 1].
        ...
        -------
        scaled_x: numpy.ndarray
            Numpy array, containing scaled and clipped x-coordinate values.
        
        scaled_y: numpy.ndarray
            Numpy array, containing scaled and clipped y-coordinate values.
 
        """
        x = np.clip(x, x_lims[0], x_lims[1])
        y = np.clip(y, y_lims[0], y_lims[1])
        scaled_x, scaled_y = CoordinateHelper.scale_positions(x, y)
        
        return scaled_x, scaled_y
    
    @staticmethod
    def add_random_shift_to_positions(xv_grid, yv_grid, grid_points, x_lims, y_lims, seed):
        ...
        xv_s, yv_s = CoordinateHelper.check_and_adjust_coordinate_limits(xv.flatten(), yv.flatten(), x_lims, y_lims)
    ...
    def check_and_adjust_coordinate_limits(x, y, x_lims, y_lims):
        ...
        A method to check and adjust coordinate limits.
...
class ZernikeHelper:
    ...
    def generate_zernike_polynomials(xv, yv, x_lims, y_lims, d_max, polynomial_coeffs):
        ...
        This method calculates Zernike polynomials based on the given x and y positions,
        ...
        -------
        numpy.ndarray
            A 1-dimensional numpy ndarray representing the Zernike polynomials generated
        from the given positions and polynomial coefficients.
        
        """     
        # Generate the polynomial matrix
        Pi_samples = PolynomialMatrixHelper.generate_polynomial_matrix(xv, yv, x_lims, y_lims, d_max)

        return polynomial_coeffs @ Pi_samples
    
    @staticmethod
    def initialize_Z_matrix(max_order, xv):
        ...
        xv: numpy ndarray
            A numpy ndarray with values representing positions.
            
        Returns
        -------
        numpy.ndarray
            An array of shape (max_order, len(xv)) containing randomly generated values
        from a standard normal distribution to initialize the Zernike matrix. 
      
        
        """
        return np.random.randn(max_order, len(xv))
    ...
    @staticmethod
    def calculate_zernike(x_lims, y_lims, xv, yv, verbose):
        ...
        return ZernikeHelper.generate_zernike_polynomials(xv_flat, yv_flat)
...
class SpatialVaryingPSF(object):
    ...
    def estimate_polynomial_coeffs(self, xv, yv):
        ...
        Z = ZernikeHelper.normalize_Z_matrix(ZernikeHelper.initialize_Z_matrix(self.max_order,xv), self.lim_max_wfe_rms)
        Pi = PolynomialMatrixHelper.generate_polynomial_matrix(xv,yv,self.x_lims,self.y_lims,self.d_max)
        
        self.polynomial_coeffs = Z @ np.linalg.pinv(Pi)
        Z_hat = self.polynomial_coeffs @ Pi
        return Z_hat
    ...
    def calculate_wfe_rms(self, xv, yv, polynomial_coeffs):
        ...
        Z=ZernikeHelper.generate_zernike_polynomials(xv,
                                                     yv,
                                                     self.x_lims, 
                                                     self.y_lims, 
                                                     self.d_max, 
    ...
    def build_polynomial_coeffs(self):
        ...
        # Build mesh
        xv_grid, yv_grid = MeshHelper.build_mesh(self.x_lims, self.y_lims, self.grid_points)
        
        # Apply random position shifts
        xv, yv = CoordinateHelper.add_random_shift_to_positions(xv_grid, yv_grid, self.grid_points, self.x_lims, self.y_lims, self.seed)
        
        # Generate Z matrix
        Z = self.estimate_polynomial_coeffs(xv, yv)
        
        ## Sampling the space
        # Choose the anchor points on a regular grid
        xv_grid, yv_grid = MeshHelper.build_mesh(self.x_lims, self.y_lims, self.grid_points, num=100, endpoint=True)
        ...
        # Due to the polynomial behaviour we will set the mean WFE_RMS over the field of view to be 80% of
        ...
        scaled_Z_estimate = ZernikeHelper.generate_zernike_polynomials(xv, yv, self.x_lims, self.y_lims, self.d_max, self.polynomial_coeffs)
        
        # Calculate and save the WFE_RMS map of the polynomial coefficients values.
        self.WFE_RMS = self.calculate_wfe_rms(xv, yv, self.polynomial_coeffs).reshape(xv_grid.shape)
    ...
    def get_monochromatic_PSF(self, xv, yv, lambda_obs=0.725):
        ...
        zernikes = ZernikeHelper.calculate_zernike(xv, yv)
