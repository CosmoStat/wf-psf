...
def simple_star_and_mask():
    ...
    image = np.zeros((num_images, 5, 5))  # Create a 3D array (5x5 image for each "image")
...
def simple_image_with_mask(simple_image):
    ...
    num_images = simple_image.shape[0]  # Get the number of images from the first dimension
...
def test_compute_centroid_correction_with_masks(mock_data):
    ...
    with patch('wf_psf.data.centroids.extract_star_data') as mock_extract_star_data, \
     patch('wf_psf.data.centroids.compute_zernike_tip_tilt') as mock_compute_zernike_tip_tilt:
...
def test_centroid_estimator_initialization(simple_image):
    ...
    assert estimator.xc0 == 2.5  # Default xc should be the center of the image, i.e. float(self.stamp_size[0]) / 2
    assert estimator.yc0 == 2.5  # Default yc should be the center of the image, i.e. float(self.stamp_size[0]) / 2
...
def test_single_iteration_auto_run(simple_image):
    ...
    with patch.object(CentroidEstimator, 'update_grid') as update_grid_mock, \
         patch.object(CentroidEstimator, 'elliptical_gaussian') as elliptical_gaussian_mock, \
        ...
        centroid_estimator = CentroidEstimator(im=simple_image, n_iter=1, auto_run=True)
...
def test_update_grid(simple_image):
    ...
    assert np.allclose(centroid_estimator.xx, 
        np.array([[[[-2.5, -2.5, -2.5, -2.5, -2.5], 
                    [-1.5, -1.5, -1.5, -1.5, -1.5],
                    [-0.5, -0.5, -0.5, -0.5, -0.5],
                    [ 0.5,  0.5,  0.5,  0.5,  0.5],
                    [ 1.5,  1.5,  1.5,  1.5,  1.5]]]]))
    ...
    assert np.allclose(centroid_estimator.yy, 
    np.array([[[[-2.5, -1.5, -0.5,  0.5,  1.5], 
                [-2.5, -1.5, -0.5,  0.5,  1.5], 
                [-2.5, -1.5, -0.5,  0.5,  1.5], 
                [-2.5, -1.5, -0.5,  0.5,  1.5], 
                [-2.5, -1.5, -0.5,  0.5,  1.5]]]]))
...
def test_intra_pixel_shifts(simple_image_with_centroid):
    ...
    assert np.isclose(shifts[0], expected_x_shift), f"Expected {expected_x_shift}, got {shifts[0]}"
    assert np.isclose(shifts[1], expected_y_shift), f"Expected {expected_y_shift}, got {shifts[1]}"
