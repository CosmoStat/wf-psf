...
def test_combine_zernike_contributions_basic_case():
    ...
    expected = np.array([[1 + 5, 2 + 0], [3 + 6, 4 + 0]])  # padded contrib2 to (2, 2)
...
def test_prior_as_tensor(mock_model_params):
    ...
    result = assemble_zernike_contributions(
        model_params=mock_model_params, zernike_prior=tensor_prior
    )
...
def test_inconsistent_shapes_raises_error(
    ...
    with pytest.raises(
        ValueError, match="All contributions must have the same number of samples"
    ):
        assemble_zernike_contributions(
            model_params=mock_model_params,
            zernike_prior=dummy_prior,
...
def test_pad_zernikes_num_of_zernikes_equal():
