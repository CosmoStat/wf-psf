...
def mock_trained_model_params():
    return MagicMock(
        model_params=MagicMock(model_name="mock_model"),
        id_name="mock_id"
    )
...
def test_evaluate_model(mock_metrics_params, mock_trained_model_params, mock_data, mock_psf_model, mocker):
    # Mock the metric functions
    with patch('wf_psf.metrics.metrics_interface.MetricsParamsHandler.evaluate_metrics_polychromatic_lowres', new_callable=MagicMock) as mock_evaluate_poly_metric, \
         patch('wf_psf.metrics.metrics_interface.MetricsParamsHandler.evaluate_metrics_mono_rmse', new_callable=MagicMock) as mock_evaluate_mono_metric, \
         patch('wf_psf.metrics.metrics_interface.MetricsParamsHandler.evaluate_metrics_opd', new_callable=MagicMock) as mock_evaluate_opd_metric, \
         patch('wf_psf.metrics.metrics_interface.MetricsParamsHandler.evaluate_metrics_shape', new_callable=MagicMock) as mock_evaluate_shape_results_dict, \
        ...
        assert mock_evaluate_poly_metric.call_count == 2  # Called twice, once for each dataset
        assert mock_evaluate_mono_metric.call_count == 2  # Called twice, once for each dataset
        ...
        assert "/mock/metrics/output/metrics-mock_modelmock_id" in output_path  # Ensure correct path format  
